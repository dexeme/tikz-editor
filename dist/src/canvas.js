...
import { NODE_RADIUS, NODE_WIDTH, NODE_HEIGHT, getNodeDimensions, getNodeBounds as computeNodeBounds, getCylinderMetrics, } from './utils/sceneMetrics.js';
...
const clampRectangleSplitParts = value => {
    ...
    return Math.max(RECTANGLE_SPLIT_MIN_PARTS, Math.min(RECTANGLE_SPLIT_MAX_PARTS, Math.round(numeric)));
};
...
const resolveColor = (candidate, fallback) => typeof candidate === 'string' && candidate.trim() ? candidate.trim() : fallback;
...
function getAnchorPoint(node, direction) {
    ...
    const totalDeg = (Number.isFinite(rotateDeg) ? rotateDeg : 0) +
...
export function createCanvasRenderer(canvas, state) {
    ...
    function drawEdge(edge) {
        ...
        if (edge.direction && edge.direction.startsWith('<')) {
            drawArrowHead(geometry.startPoint, geometry.startAngle + Math.PI, edge, strokeColor, edgeThickness);
        }
        if (edge.label?.text) {
    ...
    function drawCylinderNode(node, strokeColor, dashPattern) {
        ...
        const bodyColor = resolveColor(usesCustomFill ? node.cylinderBodyFill : null, fallbackFill);
        const endColor = resolveColor(usesCustomFill ? node.cylinderEndFill : null, fallbackFill);
        ...
        const bodyGradient = ctx.createLinearGradient(node.x, topCenterY - ry, node.x, bottomCenterY + ry);
    ...
    function drawNodeBase(node) {
        ...
        const strokeColor = (() => {
            if (isSelected && !state.borderPreviewSuppressed)
                return '#38bdf8';
            if (isTarget)
                return targetInfo.valid ? '#22c55e' : '#ef4444';
            if (isOrigin)
                return '#38bdf8';
            if (isHovered && state.edgeDraft)
                return 'rgba(96, 165, 250, 0.9)';
            if (isHovered)
                return withAlpha(baseStroke, 0.85);
        ...
        const width = (() => {
            ...
            if ((isSelected && !state.borderPreviewSuppressed) || isOrigin)
    ...
    function drawNodeHandles(node) {
        ...
        const renderAnchors = anchors.filter(anchor => {
            if (anchor.isConnectable)
                return true;
            if (state.edgeDraft?.from?.nodeId === node.id &&
            ...
            if (state.edgeDraft?.counterpart &&
        ...
        renderAnchors.forEach(({ direction, point }) => {
            ...
            else if (state.edgeDraft?.counterpart &&
                state.edgeDraft.counterpart.nodeId === node.id &&
    ...
    function drawTextBlock(block) {
        ...
        if (isSelected) {
            ...
            ctx.beginPath();
            ctx.rect(block.x + block.width - handleSize, block.y + block.height - handleSize, handleSize, handleSize);
            ctx.fill();
        }
        ctx.restore();
    }
    ...
    function getNodePath(node) {
        ...
        switch (node.shape) {
            ...
            case 'rounded rectangle':
                addRoundedRect(path, node.x - halfWidth, node.y - halfHeight, width, height, Math.max(0, node.cornerRadius ?? 16));
                break;
            case 'rectangle split': {
                const metrics = getRectangleSplitMetrics(node);
                path.rect(node.x - metrics.halfWidth, node.y - metrics.halfHeight, metrics.halfWidth * 2, metrics.halfHeight * 2);
    ...
    function drawEdgePreview() {
        ...
        if (draft.mode === 'rewire' && draft.counterpart) {
            ...
            if (!counterpartNode || !draft.pointer)
                return;
    ...
    function drawMatrixGrid(grid) {
        ...
        if (state.selected?.type === 'matrix' && state.selected.item?.id === grid.id) {
            ...
            ctx.lineWidth = 2 / scale;
            ctx.strokeStyle = '#38bdf8';
            ctx.strokeRect(grid.x - padding, grid.y - padding, width + padding * 2, height + padding * 2);
            ctx.restore();
        }
    }
    function drawScene() {
    ...
    function drawGuides() {
        ...
        if (guides.snapTarget) {
            ...
            ctx.strokeStyle = '#0f172a';
            ctx.beginPath();
            ctx.rect(guides.snapTarget.x - size / 2, guides.snapTarget.y - size / 2, size, size);
            ctx.fill();
            ctx.stroke();
            ctx.restore();
        }
        ...
        if (Array.isArray(state.spacingMeasurements) && state.spacingMeasurements.length) {
            state.spacingMeasurements.forEach(entry => drawSpacingIndicator(entry, { color: '#ef4444', dashed: true }));
        }
        ctx.restore();
    }
    ...
    function isPointInsideNode(node, point) {
        ...
        switch (node.shape) {
            ...
            default:
                return (point.x >= node.x - halfWidth &&
