// @ts-nocheck
import { buildStyleOptions, createShapeOptions, normalizeNodeParameters, } from './shapes/index.js';
import { mapPort, resolveBendShape, resolveOrthogonalTikz, isCurvedShape } from './routingMaps.js';
import { isNodeInsideFrame, formatCm, pxToCm, PX_TO_CM, resolveNodeSize, getDefaultNodeSize, } from './utils/sceneMetrics.js';
...
const shouldCompensateScale = value => Number.isFinite(value) && Math.abs(value - 1) > SCALE_EPSILON;
...
const isBlockInsideFrame = (block, frame) => {
    ...
    const bottomRightInside = isPointInsideFrame({ x: block.x + block.width, y: block.y + block.height }, frame);
    return topLeftInside && bottomRightInside;
};
export function generateTikzDocument(nodes, edges, lines = [], textBlocks = [], matrixGrids = [], frame = null, options = {}) {
    ...
    const frameOffsetX = frame && Number.isFinite(frame.x) ? Number(frame.x) : 0;
    const frameOffsetY = frame && Number.isFinite(frame.y) ? Number(frame.y) : 0;
    ...
    const filteredTextBlocks = Array.isArray(textBlocks)
        ? textBlocks
            ...
            const normalizedBorderStyle = typeof block?.borderStyle === 'string' ? block.borderStyle.trim().toLowerCase() : '';
            ...
            return {
                ...
                fillColor: typeof block?.fillColor === 'string' && block.fillColor.trim()
                    ? block.fillColor.trim()
                    : null,
                borderColor: typeof block?.borderColor === 'string' && block.borderColor.trim()
                    ? block.borderColor.trim()
                    : null,
                borderWidth: Number.isFinite(borderWidthValue) && borderWidthValue >= 0
                    ? borderWidthValue
                    : TEXT_BLOCK_BORDER_WIDTH_DEFAULT,
                borderStyle: normalizedBorderStyle === 'dashed' || normalizedBorderStyle === 'dotted'
    ...
    const filteredLines = Array.isArray(lines)
        ? lines
            ...
            color: line.color,
            style: line.style === 'dashed' || line.style === 'dotted'
                ? line.style
                : 'solid',
    ...
    filteredNodes.forEach(node => {
        ...
        const rawLabelContent = labelOverride != null ? labelOverride : formatNodeLabel(normalizedNode.label);
