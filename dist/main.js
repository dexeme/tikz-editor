// @ts-nocheck
import { createApp, ref, reactive, computed, watch, onMounted, onUnmounted, nextTick, } from 'https://unpkg.com/vue@3/dist/vue.esm-browser.prod.js';
import { createCanvasRenderer, TEXT_BLOCK_CONSTRAINTS } from './src/canvas.js';
import { DEFAULT_CYLINDER_MIN_WIDTH_CM, DEFAULT_CYLINDER_MIN_HEIGHT_CM, DEFAULT_CYLINDER_ASPECT, getDefaultNodeSize, resolveNodeSize, } from './src/utils/sceneMetrics.js';
...
function applyShapeDefaults(node) {
    ...
    const fallbackFill = (typeof node.color === 'string' && node.color.trim()) || '#f8fafc';
...
const clampRectangleSplitParts = value => {
    ...
    return Math.min(RECTANGLE_SPLIT_MAX_PARTS, Math.max(RECTANGLE_SPLIT_MIN_PARTS, Math.round(numeric)));
};
function ensureRectangleSplitCells(node) {
    ...
    for (let index = 0; index < parts; index += 1) {
        ...
        const textFallback = typeof current.text === 'string'
            ? current.text
        ...
        cells.push({
            ...
            textColor: typeof current.textColor === 'string' && current.textColor.trim()
...
function makeTextBlock(x, y, width, height, options = {}) {
    const normalizeString = (value, fallback) => typeof value === 'string' && value.trim() ? value : fallback;
    ...
    return {
        ...
        borderWidth: Math.max(0.5, normalizeNumeric(options.borderWidth, TEXT_BLOCK_BORDER_WIDTH_DEFAULT)),
...
function refreshSequencesFromState(nodes = [], edges = [], lines = [], textBlocks = [], matrixGrids = []) {
...
createApp({
    setup() {
        ...
        const activeShape = computed(() => availableShapes.find(shape => shape.id === activeShapeId.value) || availableShapes[0] || null);
        ...
        const selectedEdge = computed(() => state.selected?.type === 'edge' ? state.selected.item : null);
        const selectedLine = computed(() => state.selected?.type === 'line' ? state.selected.item : null);
        ...
        watch(() => selectedNodes.value.map(node => `${node.id}:${node.x}:${node.y}`), () => {
        ...
        const edgeToolbarHint = computed(() => {
            const showAlignmentReminder = (edgeToolbarState.hoveredOption === 'alignment' ||
        ...
        const edgeToolbarStyle = computed(() => {
            ...
            const points = [];
            if (geometry.startPoint)
                points.push(geometry.startPoint);
            if (geometry.endPoint)
                points.push(geometry.endPoint);
            if (geometry.control)
                points.push(geometry.control);
            if (geometry.elbow)
                points.push(geometry.elbow);
        ...
        const canCopyFormatting = computed(() => !!selectedNode.value || !!selectedEdge.value || !!selectedLine.value);
        const templates = [
            {
                id: 'blank-canvas',
                name: 'Blank canvas',
        ...
        function loadNextMatrixFromQueue() {
            ...
            const sortedValues = Array.from(valueSet).sort((a, b) => a.localeCompare(b, 'pt-BR', { numeric: true }));
            let paletteIndex = 0;
        ...
        function queueMatrixImports(entries, { replaceQueue = false } = {}) {
            ...
            entries.forEach(entry => {
                const rawData = Array.isArray(entry.data)
                    ? entry.data.map(row => Array.isArray(row) ? row.map(cell => String(cell)) : [])
                    : [];
                if (!rawData.length) {
                    return;
                }
        ...
        function normalizeMatrixGrid(grid = {}) {
            const rawData = Array.isArray(grid.data)
                ...
                : [];
            const columnCount = rawData[0]?.length || 0;
            const data = columnCount > 0
                ? rawData.filter(row => row.length === columnCount).map(row => [...row])
                : [];
            const colorMap = grid.colorMap && typeof grid.colorMap === 'object'
        ...
        function normalizeTextBlock(block = {}, options = {}) {
            const fallbackColor = typeof options.defaultColor === 'string' && options.defaultColor.trim()
            ...
            const colorValue = normalizeHex(typeof block.color === 'string' ? block.color : '') || fallbackColor;
            const fillValue = normalizeHex(typeof block.fillColor === 'string' ? block.fillColor : '') || null;
            const borderColorValue = normalizeHex(typeof block.borderColor === 'string' ? block.borderColor : '') || '#94a3b8';
            const borderWidthValue = Number(block.borderWidth);
            const borderWidth = Number.isFinite(borderWidthValue) && borderWidthValue >= 0
                ? borderWidthValue
                : TEXT_BLOCK_BORDER_WIDTH_DEFAULT;
            const borderStyleOptions = new Set(['solid', 'dashed', 'dotted']);
            const rawBorderStyle = typeof block.borderStyle === 'string' ? block.borderStyle.toLowerCase().trim() : '';
            ...
            const opacity = Number.isFinite(opacityValue)
                ? Math.min(TEXT_BLOCK_OPACITY_RANGE.max, Math.max(TEXT_BLOCK_OPACITY_RANGE.min, opacityValue))
                : TEXT_BLOCK_OPACITY_RANGE.max;
            return {
                id: typeof block.id === 'string' ? block.id : `text-${textSequence++}`,
                x: Number.isFinite(xValue) ? xValue : 0,
                y: Number.isFinite(yValue) ? yValue : 0,
                width: Math.max(TEXT_BLOCK_CONSTRAINTS.minWidth, Number.isFinite(widthValue) && widthValue > 0 ? widthValue : TEXT_BLOCK_DEFAULT_WIDTH),
                height: Math.max(TEXT_BLOCK_CONSTRAINTS.minHeight, Number.isFinite(heightValue) && heightValue > 0 ? heightValue : TEXT_BLOCK_DEFAULT_HEIGHT),
                text: typeof block.text === 'string' ? block.text : '',
                fontSize: Math.min(TEXT_FONT_SIZE_LIMITS.max, Math.max(TEXT_FONT_SIZE_LIMITS.min, Number.isFinite(fontSizeValue) && fontSizeValue > 0 ? fontSizeValue : 16)),
        ...
        function updateTextFontSize(value, options = {}) {
            ...
            const clamped = Math.min(TEXT_FONT_SIZE_LIMITS.max, Math.max(TEXT_FONT_SIZE_LIMITS.min, numeric));
        ...
        function updateTextOpacity(value, options = {}) {
            ...
            const normalized = Math.min(TEXT_BLOCK_OPACITY_RANGE.max, Math.max(TEXT_BLOCK_OPACITY_RANGE.min, ratio));
        ...
        function getCylinderAspectSliderValue(node) {
            ...
            const clamped = clampValue(Number.isFinite(numeric) ? numeric : DEFAULT_CYLINDER_ASPECT, 0.1, 1);
        ...
        function updateCylinderColor(key, color, options = {}) {
            const normalized = normalizeHex(color) || (typeof color === 'string' ? color.trim() : null);
            if (!normalized) {
                return;
            }
            updateCylinderProperty(key, normalized, options);
        ...
        function copySelectedFormatting() {
            ...
            if (edge) {
                ...
                formatClipboard.value = {
                    ...
                    thickness: Number.isFinite(rawThickness) && rawThickness > 0 ? rawThickness : null,
                };
        ...
        function pasteSelectedFormatting() {
            ...
            if (payload.type === 'node') {
                ...
                const borderStyle = typeof payload.borderStyle === 'string' ? payload.borderStyle : 'solid';
        ...
        async function handleMatrixFileChange(event) {
            ...
            if (!files.length)
                return;
            try {
                const sortedFiles = files.sort((a, b) => a.name.localeCompare(b.name, 'en-US', { numeric: true, sensitivity: 'base' }));
                const successes = [];
                const failures = [];
                ...
                if (successes.length) {
                    ...
                    const combinedMessage = failures.length === 1
        ...
        function applyDiagramPayload(payload) {
            ...
            const textBlocks = Array.isArray(payload.textBlocks)
                ? payload.textBlocks.map(block => normalizeTextBlock({ ...block }, { defaultColor: defaultTextColor.value }))
                : [];
        ...
        const defaultTextColor = computed(() => state.theme === 'dark' ? '#e2e8f0' : '#0f172a');
        ...
        const isPreviewDirty = computed(() => !!tikzCode.value && tikzCode.value !== lastRenderedTikz.value);
        ...
        function buildTikzPreviewSrcdoc(rawCode) {
            ...
            const scriptContents = sanitizeScript(normalizeAccents([sanitizedPreamble, body].filter(Boolean).join('\n\n')));
            return [
                '<!DOCTYPE html>',
                '<html lang="pt-BR">',
                '<head>',
        ...
        function applyPreviewBounds(bounds) {
            ...
            const padding = Math.max(PREVIEW_PADDING_MIN, Math.min(PREVIEW_PADDING_MAX, baseSize * PREVIEW_PADDING_RATIO));
        ...
        const canReset = computed(() => state.nodes.length > 0 ||
        ...
        function invalidateTikz(options = {}) {
            ...
            tikzCode.value = generateTikzDocument(state.nodes, state.edges, state.lines, state.textBlocks, state.matrixGrids, state.frame, {
                edgeThickness: state.edgeThickness,
                edgeLabelAlignment: state.edgeLabelAlignment,
            });
            tikzUpdatePending.value = false;
        }
        watch(() => [state.nodes, state.edges, state.lines, state.textBlocks, state.matrixGrids], () => {
            invalidateTikz();
        }, { deep: true, immediate: true });
        watch(() => state.edgeThickness, () => {
            invalidateTikz();
        });
        watch(() => state.edgeLabelAlignment, () => {
            invalidateTikz();
        });
        ...
        watch(() => matrixPrompt.visible, visible => {
            if (visible) {
                nextTick(() => {
                    matrixPromptTextAreaRef.value?.focus();
                });
        ...
        watch(() => matrixPrompt.text, () => {
            if (matrixPrompt.error) {
                matrixPrompt.error = '';
            }
        });
        watch(() => state.selected?.item?.id, () => {
            renderer.value?.draw();
        });
        ...
        function createActiveShape(options = {}) {
            ...
            const position = options.position || state.pointer || getViewportCenterWorld();
            createNodeAtCenter(shapeId, { ...options, position });
        }
        function createTextBlock(options = {}) {
            closeFormsMenu();
            closeDiagramMenu();
            const target = options.position || state.pointer || getViewportCenterWorld();
            const width = Math.max(TEXT_BLOCK_CONSTRAINTS.minWidth, Number(options.width) || TEXT_BLOCK_DEFAULT_WIDTH);
            const height = Math.max(TEXT_BLOCK_CONSTRAINTS.minHeight, Number(options.height) || TEXT_BLOCK_DEFAULT_HEIGHT);
            const color = typeof options.color === 'string' && options.color.trim()
                ? options.color.trim()
                : defaultTextColor.value;
            const fillColor = typeof options.fillColor === 'string' && options.fillColor.trim()
                ? options.fillColor.trim()
                : state.theme === 'dark'
                    ? '#1f2937'
                    : '#f8fafc';
            const borderColor = typeof options.borderColor === 'string' && options.borderColor.trim()
                ? options.borderColor.trim()
                : '#94a3b8';
            const block = makeTextBlock(target.x - width / 2, target.y - height / 2, width, height, {
                color,
                fillColor,
                borderColor,
        ...
        function applyTemplate(templateId) {
            ...
            if (requiresConfirmation) {
                const confirmed = window.confirm('Loading a template will replace the current diagram. Continue?');
                if (!confirmed) {
                    return;
                }
            }
            ...
            state.lines = Array.isArray(template.lines)
                ...
                : [];
            state.textBlocks = (template.textBlocks || []).map(block => normalizeTextBlock({ ...block }, { defaultColor: defaultTextColor.value }));
            state.matrixGrids = (template.matrixGrids || []).map(grid => normalizeMatrixGrid({ ...grid }));
            state.selected = null;
            state.mode = 'move';
            state.edgeDraft = null;
            state.hoverNodeId = null;
        ...
        function setSelected(payload, options = {}) {
            ...
            if (payload.type === 'node') {
                ...
                const primary = payload.item && items.some(item => item.id === payload.item.id)
                    ? payload.item
        ...
        function applyGuides(optionsOrVertical, maybeHorizontal) {
            if (!state.guides)
                return;
            if (typeof optionsOrVertical === 'object' &&
                optionsOrVertical !== null &&
                maybeHorizontal === undefined) {
                const { vertical = null, horizontal = null, snapTarget = null, rotation = null, spacing = null, } = optionsOrVertical;
        ...
        function collectStaticBounds(context) {
            ...
            (state.textBlocks || []).forEach(block => {
                if (context.mode === 'move-text' && block.id === context.item?.id)
                    return;
                pushBounds(renderer.value.getTextBlockBounds(block));
            });
            (state.matrixGrids || []).forEach(grid => {
                if (context.mode === 'move-matrix' && grid.id === context.item?.id)
                    return;
        ...
        function computeAxisGaps(boundsList, axis) {
            const sorted = [...boundsList].sort((a, b) => axis === 'x' ? a.left - b.left : a.top - b.top);
            const gaps = new Set();
            for (let index = 0; index < sorted.length - 1; index += 1) {
                ...
                if (!current || !next)
                    continue;
                const gap = axis === 'x' ? next.left - current.right : next.top - current.bottom;
        ...
        function onCanvasMouseDown(event) {
            ...
            if (canStartEdgeFromAnchor) {
                ...
                if (selectedEdge) {
                    const isFromMatch = selectedEdge.from === anchorHit.node.id && selectedEdge.fromAnchor === anchor;
                ...
                if (!selectedEdge) {
                    const connected = state.edges.filter(edgeItem => {
                        return ((edgeItem.from === anchorHit.node.id && edgeItem.fromAnchor === anchor) ||
        ...
        function applySnapshot(snap) {
            ...
            state.textBlocks = snap.textBlocks.map(block => normalizeTextBlock({ ...block }, { defaultColor: defaultTextColor.value }));
            state.matrixGrids = (snap.matrixGrids || []).map(grid => normalizeMatrixGrid({ ...grid }));
        ...
        function saveDiagram() {
            ...
            const requestedName = window.prompt('Enter the file name (without extension):', defaultName);
            const sanitizedName = (() => {
